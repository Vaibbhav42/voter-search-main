...
const SearchVoters = ({ selectedDataset, navigation }) => {
  ...
  const handleSearchTextChange = (text) => {
    ...
    setTimeout(() => {
      ...
      const filtered = allVoters.filter(voter => 
        ...
        (voter.id && voter.id.toLowerCase().includes(query)) ||
        ...
        (voter.address && voter.address.toLowerCase().includes(query)) ||
        (voter.boothNo && voter.boothNo.toLowerCase().includes(query)) ||
        (voter.gender && voter.gender.toLowerCase().includes(query))
      );
  ...
  const toggleResponse = (voter, event) => {
